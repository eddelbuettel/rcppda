...
// Copyright 2016-2023 Dirk Eddelbuettel <edd@debian.org>
//
// For changes enabing use with Rcpp
// Replaces fprintf(stderr, ...) with REprintf()
// and ::abort() with Rcpp::stop
...
struct default_handler
{
                       ...
                       const char* message = nullptr) noexcept
    {
        ...
        {
            if (message)
                REprintf("[debug assert] %s:%u: Unreachable code reached - %s.\n",
                         loc.file_name, loc.line_number, message);
            else
                REprintf("[debug assert] %s:%u: Unreachable code reached.\n",
                         loc.file_name, loc.line_number);
        }
        else if (message)
            REprintf("[debug assert] %s:%u: Assertion '%s' failed - %s.\n", loc.file_name,
                     loc.line_number, expression, message);
        else
            REprintf("[debug assert] %s:%u: Assertion '%s' failed.\n", loc.file_name,
...
#endif
        return Handler::handle(loc, expression, detail::forward<Args>(args)...),
          Rcpp::stop(expression),  // instead of ::abort(),
          regular_void();
#if defined(_MSC_VER)
#    pragma warning(pop)
#endif
